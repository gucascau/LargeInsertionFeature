...
foreach my $m (keys %str){
	
	if (exists $str{$m} && $hash{$m}){
		print O "$m\tOverlapping\n";
	}elsif (exists $str{$m} && !exists $hash{$m}){
		print O "$m\tIndexSpecific\n";
	}elsif (exists $hash{$m} && !exists $str{$m}){
		print O "$m\tRequestSepcific\n";
	}
