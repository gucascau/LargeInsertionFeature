...
helpFunction()
{
	...
	echo "Usage: sh $0 -a Sample ID -b Work Directory -c Ty1 Reference -d Ty1 Annotation -f Insertion events -r Output  -p Software installed Directory"
	...
	echo -e "\t-c Ty1 blast index (Note: if you don't have blast index, please run the 'makeblastdb -in Ty1.fasta -dbtype nucl')"
	echo -e "\t-d The Ty1 annotation file"
	...
	echo -e "\t-o the output folder (The output files will contain the Ty1 nucleotide of insertions, including the overall, forward alignments and reverse alignments)"
	echo -e "\t-p Software installed Path (Default:"", it is the path where you store your blast software)" 
	echo -e "\t-r Script Stored Path" 
...
while getopts "a:b:c:d:p:f:r:o:ms:mc:mb" opt
do
   case "$opt" in
      a ) SampleID="$OPTARG" ;;
      b ) WDir="$OPTARG" ;;
...
if [ -z "${TYIndex}" ] 
then
   echo "*** error: input Ty1 reference blastn index events must be defined ***";
   helpFunction
fi

if [ -z "${TYAnn}" ] 
then
   echo "*** error: input Ty1 annotation must be defined ***";
   helpFunction
fi
...
if [ -z "${Dscript}" ] 
then
   echo "*** error: input Software script directory must be defined ***";
   helpFunction
fi
...
cat ${Insert}| perl -ne '{chomp; my ($read,$id,$chr,$start,$end,$strand,$seq,$inf,$ltr,$distdes)=(split/\t/,$_)[1,2,10,11,12,13,40,41,42,43];next if ($inf eq "NO");   next if ($chr eq "chrXII" && $start >=451418 && $end <= 469316); my ($type,$details)=(split/\|/,$inf)[0,1];next unless (($type eq "LTR_retrotransposon" && $distdes eq "ENTIRE" && $details =~/Ty1-1/)); print ">$read\n$seq\n" }'  > ${SampleID}.LTR.fasta
 
# print the insertion events:

cat ${Insert}| perl -ne '{chomp; my ($read,$id,$chr,$start,$end,$strand,$seq,$inf,$ltr,$distdes)=(split/\t/,$_)[1,2,10,11,12,13,40,41,42,43];next if ($inf eq "NO");   next if ($chr eq "chrXII" && $start >=451418 && $end <= 469316); my ($type,$details)=(split/\|/,$inf)[0,1];next unless (($type eq "LTR_retrotransposon" && $distdes eq "ENTIRE" && $details =~/Ty1-1/)); print "$_\n" }'  > ${SampleID}.Ty1.insert.txt
...
perl ${srcDir}/CalculateCoverage_Retrotransposon.pl -i ${SampleID}.LTR.uniq.sorted.bed -a ${Ty1Ann} -b ${Ty1} -g ${SampleID} -o ${OutputDir}/${SampleID}.LTR.cov.txt
